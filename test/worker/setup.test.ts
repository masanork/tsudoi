...
describe("initial administrator setup", () => {
  it("requires a display name and stores an optional normalized email with the passkey challenge", async () => {
    ...
    const options = await SELF.fetch("https://tsudoi.test/api/setup/initial-admin/options", {
      method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ displayName: "Tsudoi Admin", email: " ADMIN@Example.Test " }),
    });
    ...
    await expect(env.DB.prepare("SELECT organization_name, display_name, email_normalized FROM initial_admin_challenges WHERE id = ?").bind(challengeId).first())
      .resolves.toEqual({ organization_name: "既定ワークスペース", display_name: "Tsudoi Admin", email_normalized: "admin@example.test" });
  });
